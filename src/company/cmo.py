...
# Autonomous fallback channels — only things the AI company can do internally.
_DEFAULT_CHANNELS = [
...
CRITICAL: Hermes is a fully autonomous AI company. We have NO access to the owner's email, phone, printer, external contacts, human social networks, or ability to make real-world partnerships. We CAN execute actions using only our own internal capabilities: operators, call data, training/simulation, self-generated scripts, automated reports, A/B testing in simulation, and word-of-mouth from existing families.

DO NOT propose channels that require a human to send emails, make calls, print flyers, attend meetings, or negotiate partnerships. Propose channels that can run autonomously:
- product-led growth from existing families (shareable automated reports, referral prompts during calls),
- autonomous digital presence (generated landing-page copy, SEO content, social posts if API access existed — but only if we can generate them internally),
- call-data insights (identify high-satisfaction moments, turn them into testimonials/case studies),
- internal simulation experiments (test scripts, offers, messaging in self-play before exposing real seniors).

Be creative and realistic about what an autonomous AI company can actually do. 3-5 channels, 2-3 segments, 3-4 next steps, all internally executable.
